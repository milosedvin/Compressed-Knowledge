    ...
    <v-container fluid>
        <v-row>
            <v-col offset-sm="1" sm="6">
                ...
                <div class="pa-2" id="info">
                    <v-row class="ma-0">
                        <v-col cols="12" md="1" class="pa-2">
                            <p class="font-weight-bold body-1 pl-1 pink--text white">
                                INFO:
                            </p>
                        </v-col>
                        ...
                        <v-col cols="12" md="6" class="pa-2">
                            ...
                            <v-row>
                                ...
                                <span class="font-weight-light caption pl-1">
                                    ...
                                    Hide the course. So no one will see how much time you have actually wasted
                                </span>
                            </v-row>
                        </v-col>
                    </v-row>
                ...
                <p class="font-weight-bold body-1 pa-3 pink--text white">
                     ORDERS:
                </p>
                <v-simple-table>
                <template v-slot:default>
                    ...
                    <tr v-for="item in orderItems" :key="item.name">
                        ...
                        <td class="py-3">
                            <p style="margin:0;" v-for="item in item.orderLines" :key="item.id">{{ item.quantity }}</p>
                        </td>
                        <td class="py-3">
                            <p style="margin:0;" v-for="item in item.orderLines" :key="item.id">{{ item.name }}</p>
                        </td>
                        <td class="py-3">
                            <p style="margin:0;" v-for="item in item.orderLines" :key="item.id">{{ item.price }}</p>
                        </td>
